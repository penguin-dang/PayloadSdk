...
int main(int argc,char** argv){
    ...
    while (true) {
        ...
        if (ready_click) {
            // Draw a horizontal line through the center
            cv::line(frame, cv::Point(0, mouse_move_y), cv::Point(frame.cols, mouse_move_y), cv::Scalar(255, 0, 0), 1);
            // Draw a vertical line through the center
            cv::line(frame, cv::Point(mouse_move_x, 0), cv::Point(mouse_move_x, frame.rows), cv::Scalar(255, 0, 0), 1);
        }
